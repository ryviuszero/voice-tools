...
name: OpenVoiceOS
slug: openvoiceos
tagline: Open-source privacy-focused voice assistant platform for custom devices
website: 'https://www.openvoiceos.org/'
logo: /logos/openvoiceos.ico
...
pricing:
  ...
  has_free_tier: true
  starting_paid_usd: 0
  pricing_url: 'https://github.com/OpenVoiceOS'
licensing:
  commercial_use: true
...
i18n:
  zh:
    ...
    body: >-
      ...
      OpenVoiceOS 适合优先评估的场景是：设备端隐私优先语音助手。它的核心价值在于 自定义智能音箱、Linux
      设备和本地语音界面，不要把它当成覆盖所有语音需求的万能平台。

      ### 场景越清楚，越容易判断是否值得用

      如果你的输入、输出和审核步骤都已经明确，OpenVoiceOS
      会更容易发挥价值。比如你知道要处理哪类音频、生成什么格式、交给谁审核，再决定是否放进长期流程。

      ### 边界条件才是新手最容易踩坑的地方

      如果你需要销售电话自动化平台，不要直接把它定为默认工具。先和页面中的替代方案比较，再投入设置时间、额度、声音资产或团队培训成本。
      ...
      OpenVoiceOS 给你更多技术控制权，但安装、升级、模型选择和稳定性都要自己负责。
      ...
      OpenVoiceOS 应该提高生产效率，而不是替代审核。最稳的第一步是：准备真实样本，先跑短片段，检查输出，再调整设置，最后批量处理。
...
video:
  provider: youtube
  url: 'https://www.youtube.com/watch?v=hCwdtZu7WqA'
  title: OpenVoiceOS Voice Assistant Platform Showcase
  i18n:
    zh:
      title: OpenVoiceOS 语音助手平台展示
github_metrics:
